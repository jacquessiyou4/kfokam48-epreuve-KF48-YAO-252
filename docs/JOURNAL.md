...
**IA :** a relu le dépôt contre le barème et écrit les tests. Vérifié : les tests échouent quand on casse volontairement le code testé (code d'erreur ignoré dans le client, moyenne arrondie à l'écran : 3 tests rouges), puis repassent ; `npm run lint`, `npm run build` et le `docker build` du frontend passent ; aucun hash de `main` antérieur n'a changé.
